...
export function specToCode(spec, options = {}) {
  ...
  if (neededRuntimeExports.length > 0) {
    // Unlike "d3" (resolved by whatever bundler/import-map/CDN loads this
    // file), "./vl2d3-runtime.js" is a *sibling file from this project*, not
    // a published package -- copying just this one file elsewhere (a gist,
    // a different directory) silently breaks it unless that copy travels
    // with it too.
    lines.push(
      '// NOTE: this file depends on vl2d3-runtime.js (a copy of vl2d3\'s own src/runtime.js) --',
      '// it is not a fully standalone snippet; keep that file alongside this one.',
      `import {${neededRuntimeExports.join(', ')}} from "./vl2d3-runtime.js";`
    );
  }
