...
Every generated file opens with a `// Generated by vl2d3.vegaLiteToD3Code(spec, ...)`
comment. `includeSourcePaths: true` (default `false`) additionally labels
blocks of the generated code with the JSON path in the *input* spec they
came from (e.g. `// from: mark, encoding.x, encoding.y`, `// from:
layer[0].transform`) — but at coarser granularity than `vl2altair`/
`vl2vlapi`'s own per-channel comments, because `buildUnitOrLayerBody()`'s
own "shared scales" section (above) builds one `x`/`y`/`color`/... scale
*per channel*, not *per layer child* — a scale can legitimately be fed by
several children at once (e.g. two layers sharing one y-axis), so there's
no single JSON path a comment above that one `const y = d3.scaleLinear(...)`
line could honestly point to. Rather than fake a path or omit the scale
lines entirely, this stops at three coarser, still-genuinely-single-owner
points instead: a `// from: layer[i]` boundary comment opening each child's
own block of data-prep statements, `// from: layer[i].transform` before
that child's own top-level `transform` array runs, and one combined
`// from: layer[i].mark, layer[i].encoding.x, ...` comment immediately
before the block of code that actually draws that child's mark (every
encoding channel it reads is inlined directly into that same block anyway,
e.g. `x(d["field"])`, so listing them alongside "mark" is the accurate
level to annotate at). `prepare.js`'s own inline aggregate/bin/timeUnit
handling (a separate pipeline from the top-level `transform` array, see
above) isn't annotated at all — a deliberate scope cut, not an oversight.

Paths are relative to the *view* being rendered (a unit or layer chart) --
threaded as a plain string parameter alongside the existing `ignoreUnsupported`
boolean through the same call graph (`buildUnitOrLayerBody`,
`buildPanelFunction`, `buildRuntimeFacetPanels`, `buildRuntimeFacetGrid`),
mirroring how that flag already reaches every one of those functions.
Nested inside a facet/repeat/concat panel, a view's own comments don't
carry that outer composition's own prefix (its own panel-building
functions don't thread a path *into* `buildUnitOrLayerBody()`, only the
on/off flag) — accurate locally, just not fully qualified from the spec
root, a scoping call made given how much further this project's own
composition-building machinery would otherwise need touching.
