...
Every generated script opens with a `# Generated by vl2altair.vegalite_to_
altair_code(spec, ...)` comment showing the exact call (and arguments) that
produced it — a reminder that the file is machine-generated, not something
to hand-edit and expect to survive the next re-run.

`include_source_paths=True` (default `False`) goes further: it precedes
each of `Emitter`'s own statements with a `# from: <json path>` comment
naming the part of the *input* spec that produced it (e.g. `# from:
encoding.x, encoding.y`, `# from: transform[0]`, `# from: layer[1].mark`).
This falls out naturally from the one-statement-per-operation design above
— every `add_stmt()` call site already corresponds to exactly one spec
property (or, for `.encode(...)`'s combined call, one small group of
sibling channels), so the path is just threaded alongside `hint` through
the same `translate_spec`/`_translate_unit`/`_translate_layer`/... dispatch
chain, accumulating a `layer[i].`/`spec.`/... prefix at each composition
boundary. The one deliberate gap: a facet/repeat/concat child's own path
resets to empty rather than carrying its parent composition's prefix,
since `_translate_facet`/`_translate_repeat`/`_translate_multi` don't
thread `path` into their own recursive `translate_spec` calls — a scoping
call made to keep the threading mechanical rather than exhaustive.
