...
vegalite_to_ggplot <- function(spec, chart_var = "chart", ignore_unsupported = FALSE) {
  ...
  # A provenance comment: this script is machine-generated from a Vega-Lite
  # spec, not hand-written -- re-run the translator (with the same arguments
  ...
  header <- c(
    ...
    "library(ggplot2)",
    if (needs_vl2ggplot) {
      # Unlike ggplot2/dplyr (real CRAN packages), vl2ggplot itself is this
      # project's own package -- a script calling one of its exported helper
      # functions (vl_pivot()/vl_truthy()/...) below is not a standalone
      # ggplot2 script; it needs vl2ggplot installed (`R CMD INSTALL`, see
      # this project's own README) to actually run.
      c(
        "# NOTE: this script depends on helper functions from the vl2ggplot package itself",
        "# (vl_pivot()/vl_truthy()/... -- see R/runtime.R); it is not a standalone ggplot2 script.",
        "library(vl2ggplot)"
      )
    },
    ""
