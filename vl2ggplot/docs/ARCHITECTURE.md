...
Every generated script opens with a `# Generated by vl2ggplot::vegalite_to_ggplot(spec, ...)`
comment. `include_source_paths = TRUE` (default `FALSE`) additionally
labels blocks of the generated script with the JSON path in the *input*
spec they came from — the same idea, and the same coarser-than-per-channel
scoping decision, as `vl2d3`'s own equivalent feature (see its own
`docs/ARCHITECTURE.md`): a `# from: layer[i]` boundary comment opening each
layer child's own block of data-prep statements, `# from: layer[i].transform`
before that child's own top-level `transform` array runs, and one combined
`# from: layer[i].mark, layer[i].encoding.x, ...` comment before the
`geom_*(...)` call whose own `aes()` mapping reads every one of those
channels at once (mirroring `vl2altair`/`vl2vlapi`'s identical treatment of
their own single combined `.encode(...)` call).

Threading this through turned out simpler here than in `vl2d3`, for a
reason specific to R: `new_emitter()` returns a mutable environment, so the
on/off flag itself (`emitter$include_source_paths`) needs setting only
*once*, at the root — any function holding a reference to that same
`emitter` can read it directly (`emit_from()`, `translator.R`) without it
being threaded as a parameter through every function signature the way
`ignore_unsupported` (and `vl2d3`'s own `includeSourcePaths`, a genuine
value not a shared reference) has to be. Only the JSON-path *string* itself
needs threading, and only through the handful of functions that actually
build a source-path comment (`prepare_unit()`, `translate_unit()`,
`translate_layer()`) — everything downstream of those (`geoms.R`,
`transforms.R`, ...) never needed to know this feature exists at all.

Paths are relative to the *view* being rendered, same as `vl2d3`: nested
inside a facet/repeat/concat panel, a view's own comments don't carry that
outer composition's own prefix (`translate_facet()`/`translate_repeat()`/
`translate_multi()` don't thread `path` into their own recursive
`translate_spec()` calls) — accurate locally, not fully qualified from the
spec root, the same scoping call made for the same reason.
